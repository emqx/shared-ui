export const zhIntegrationDesc: Record<string, Record<string, string>> = {
  common: {
    ...
    max_linger_time:
      '每个分区生产者为将更多消息累积成更大批次而等待的最长时间。<br/>默认值 <code>0</code> 表示不等待（消息到达后立即发送）。<br/>一旦累积的数据达到完整批次大小，等待将提前结束，因此繁忙的生产者不会被延迟。<br/>使用磁盘缓冲时，等待发生在写入缓冲区之前；建议至少配置 <code>5ms</code> 以减少磁盘 IOPS。<br/>部分 EMQX 版本在使用内存缓冲时会忽略此配置，不进行等待。具体受影响版本请参阅版本兼容性说明。',
    max_linger_bytes:
      '每个分区生产者在等待期间可累积的最大字节数；达到该值后将停止等待并发送批次。',
  ...
  disk_log: {
    filepath:
      '待写入日志文件的基础路径。实际日志文件采用 `filepath.N` 格式，其中 `N` 的范围为 `1..max_file_number`。启用基于时间的轮转（`rotation.period`）后，当前周期的日期标记还会插入文件扩展名之前：例如 `filepath = /var/log/emqx/mqtt-trace.log` 时，文件名为 `mqtt-trace-2026062400.log.N`。可通过最近修改时间确定当前使用的文件。包含该文件的目录必须允许 EMQX 应用用户写入，因为目录中还会包含以 `.siz` 和 `.idx` 结尾的内部文件。',
    max_file_size:
      '当前活动日志文件的最大大小。每个日志文件至少会写入一条记录，因此如果单条日志记录超过此值，最终文件大小可能会超过此最大值。',
    max_file_number:
      '使用的最大日志文件数量。启用基于时间的轮转（`rotation.period`）后，此限制分别应用于每个轮转周期的文件集。达到最大文件数量且需要再次轮转时，最旧文件的内容会被丢弃，该文件将作为新的当前文件重复使用。',
    rotation:
      '用于在日或小时等日历周期的边界轮转到一组新的日志文件，将周期日期编码到文件名中，并清理旧周期的文件。',
    rotation_period:
      '轮转周期选择**每天**（`day`）或**每小时**（`hour`）时，连接器会在每个周期边界启动一组新的日志文件，并将周期日期标记（`YYYYMMDDHH`）插入文件扩展名之前。例如 `filepath = /var/log/emqx/mqtt-trace.log` 时，**每小时**轮转的文件名为 `mqtt-trace-2026062413.log.N`，**每天**轮转的文件名为 `mqtt-trace-2026062400.log.N`（每日标记的小时固定为 `00`，因此之后切换轮转周期时文件名格式保持不变）。`.N` 后缀中的 `N` 范围为 `1..max_file_number`，由底层循环日志生成且始终存在；每个周期的文件集还包含 `.idx` 和 `.siz` 管理文件。一个周期内仍会按大小（`max_file_size`）轮转，因此应根据最坏情况下单个周期的数据量设置 `max_file_size × max_file_number`，避免周期内循环覆盖导致数据丢失。连接器在健康检查时检测周期边界，所以新周期文件会在边界后的一个 `health_check_interval` 内出现。选择**仅按文件大小轮转**（`none`，默认值）时，不启用基于时间的轮转。',
    rotation_retention_period:
      '上一轮转周期的文件保留时长。每次周期轮转后，早于此时长的带日期标记日志文件会被删除，包括对应的 `.idx` 和 `.siz` 管理文件。默认值 `infinity` 表示永久保留。当轮转周期选择**仅按文件大小轮转**（`none`）时，此设置不生效。',
    rotation_timezone:
      '用于确定周期边界和日期标记的时区。可设置为 `UTC`（默认值）、`local`（服务器本地时间）或 `+02:00` 等固定 UTC 偏移量。',
...
- TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256`,
  ...
  zerobus: {
    zerobus_endpoint: 'Databricks Zerobus 数据摄入服务端点，必须指定端口号。',
    authentication: '用于 Databricks 工作区认证的 OAuth 2.0 客户端凭据。',
    workspace_id: 'Zerobus 使用的 Databricks 工作区 ID。',
    workspace_url: 'Databricks 工作区 URL，EMQX 使用它构造认证端点。',
    client_id: '用于工作区认证的 OAuth 客户端 ID。',
    client_secret: '用于工作区认证的 OAuth 客户端密钥。',
    authentication_timeout: '请求认证令牌的超时时间。',
    authentication_ssl: '连接认证端点时使用的 TLS 配置。',
    ssl: '连接 Zerobus 数据摄入服务时使用的 TLS 配置。',
    transport: '连接 Zerobus 使用的传输类型。gRPC 支持 JSON 和 Protobuf，REST 仅支持 JSON。',
    transport_connect_timeout: '建立 Zerobus 连接的超时时间。',
    transport_pool_size: 'Zerobus 连接池中的连接数。',
    transport_pipelining: 'REST 连接上无需等待响应即可连续发送的 HTTP 请求数量上限。',
    transport_max_inactive: 'REST 连接空闲多久后关闭。',
    catalog: '目标表所属的目录名称。',
    schema: '目标表所属的模式名称。',
    table: 'Databricks 中的目标表名称。',
    record:
      '发送到 Zerobus 的记录格式。REST 传输仅支持 JSON；Protobuf 需要在 Schema Registry 中注册对应的 Schema。',
    schema_name: 'EMQX Schema Registry 中的 Protobuf Schema 名称。',
    message_type: '编码数据时使用的 Protobuf 消息类型。',
  },
}
